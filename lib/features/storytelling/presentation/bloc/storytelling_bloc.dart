...
class StorytellingBloc extends Bloc<StorytellingEvent, StorytellingState> {
  ...
  Future<void> _onLoadStories(
    ...
    result.fold(
      (failure) {
        emit(StorytellingError(message: 'Failed to load stories: ${failure.message}'));
      },
      (stories) => emit(StoriesLoaded(stories)),
    );
  }
  ...
  Future<void> _onLoadVocabulary(
    ...
    result.fold(
      (failure) => emit(StorytellingError(message: 'Failed to load vocabulary: ${failure.message}')),
      (vocabulary) => emit(VocabularyLoaded(vocabulary)),
    );
  }
  ...
  Future<void> _onDetectEmotion(
    ...
    emit(EmotionLoading());
    try {
      // Optionally verify emotion with remote data source (uncomment if needed)
      // final verifiedEmotion = await detectEmotion(event.frames);
      // if (verifiedEmotion != event.emotion) {
      //   throw Exception('Emotion verification failed');
      // }
      emit(EmotionDetected(emotion: event.emotion, storyId: event.storyId));
    } catch (e) {
      emit(StorytellingError(message: 'Failed to detect emotion: $e'));
    }
  }
  ...
  Future<void> _onChangeStory(
    ...
    final result = await changeStory(event.storyId);
    result.fold(
      (failure) => emit(StorytellingError(message: 'Failed to change story: ${failure.message}')),
      (story) => emit(StoryUpdated(story: story)),
    );
  }
}