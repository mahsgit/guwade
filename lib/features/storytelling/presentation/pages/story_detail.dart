...
//     _initTts();
//     availableCameras().then((cameras) => _initCamera(cameras));
...
//   Future<void> _initCamera(dynamic cameras) async {
//     if (cameras.isEmpty) {
...
class _StoryDetailPageState extends State<StoryDetailPage> {
  ...
  Future<void> _initCamera(List<CameraDescription> cameras) async {
    ...
    try {
      ...
      setState(() => _emotion = "No front camera available");
      return;
    }
  ...
  Future<void> _captureAndSendFrames() async {
    ...
    try {
      // Capture 15 frames at 100ms intervals
      for (int i = 0; i < 15; i++) {
        XFile file = await _cameraController!.takePicture();
        capturedFrames.add(file);
        await Future.delayed(const Duration(milliseconds: 100));
      ...
      // Convert frames to bytes for the bloc and remote data source
      final List<Uint8List> bytesList = await Future.wait(capturedFrames.map((file) => file.readAsBytes()));

      // Send frames to backend and get response
      final emotion = await sendFramesToServer(capturedFrames);

      // Dispatch emotion to StorytellingBloc
      context.read<StorytellingBloc>().add(DetectEmotion(
        emotion: emotion,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<StorytellingBloc, StorytellingState>(
        listener: (context, state) {
          if (state is EmotionDetected) {
            setState(() => _emotion = state.emotion);
            if (state.emotion == 'sad' && !_suppressSadPopup) {
              _showBoredDialog();
            }
          } else if (state is StoryUpdated) {
            setState(() {
          ...
          } else if (state is StorytellingError) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () {
              setState(() => _suppressSadPopup = true);
              Navigator.pop(context);
            },
            child: const Text(
              'No',
